  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <View style={AppStyles.contactImageContainer}>
          {item?.hasThumbnail ? (
            ...
            <Text style={AppStyles.semibold25}>{getInitials(item.givenName, item.familyName)}</Text>
          )}
        </View>
        <View>
      ...
      {check ? (
        ...
        >
          <AssetImage asset={Assets.checkSign} width={normalize(13)} height={normalize(12)} />
        </LinearGradient>
      ) : (
        <View
          style={{
