...
const GroupListItem = ({ item, index  }) => {
    ...
    const UnreadDot = () => {
        if (!readByUser) {
          return <LinearGradient style={styles.unreadDot} colors={Colors.mainGradient}></LinearGradient>;
        } else {
          return null;
        }
    }
      ...
      >
        ...
        <View style={styles.messageDetails}>
          <View style={styles.nameTimeRow}>
            ...
            <Text style={[AppStyles.medium13, {fontWeight: weight()}]}>{formatDate(item.modifiedAt)}</Text>
          </View>
          <View style={styles.songDotRow}>
            <View style={styles.songData}>
              <Text numberOfLines={2} style={[AppStyles.medium13, {fontWeight: weight(), color: color()}]}>{item.recentMessage.text}</Text>
            </View>
            <UnreadDot />
          </View>
        </View>
